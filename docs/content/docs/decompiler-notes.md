...
String switches lower even further from the source shape: HashLink first null-checks the scrutinee, then guards on `s.length` before ever comparing content, then chains `std.string_compare(s.bytes, "case", len) == 0` calls for each case, and every one of those three checks jumps to the next case when it fails. Lifted as ordinary conditionals, that next case would be copied under each of the three failure edges, and again for every case after it - IR that grows 3^n with the number of cases whenever the case bodies fall through to a shared end. So `IRFunction._match_string_switch` recognizes the null-check/length-guard/compare-chain shape directly in the CFG, and `_lift_string_switch` lifts it as one `IRSwitch` keyed by the literal case strings, lifting each case body once.
