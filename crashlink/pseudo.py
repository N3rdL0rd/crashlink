...
def _expression_to_haxe(expr: IRExpression, code: Bytecode, ir_function: IRFunction) -> str:
    if isinstance(expr, IRLocal):
        ...
        op_map = {
            ...
            IRBoolExpr.CompareType.LT: "<", IRBoolExpr.CompareType.LTE: "<=",
            IRBoolExpr.CompareType.GT: ">", IRBoolExpr.CompareType.GTE: ">=",
        }
...
def _generate_statements(
    ...
    for stmt in statements:
        if isinstance(stmt, IRBlock): # Nested block, usually from if/else/loop bodies
            ...
            output_lines.extend(
                _generate_statements(stmt.statements, code, ir_function, indent_level, declared_vars_in_scope.copy())
            )
            ...
            output_lines.extend(
                _generate_statements(stmt.true_block.statements, code, ir_function, indent_level + 1, declared_vars_in_scope.copy())
            )
            if stmt.false_block and stmt.false_block.statements:
                output_lines.append(f"{indent}}} else {{")
                output_lines.extend(
                    _generate_statements(stmt.false_block.statements, code, ir_function, indent_level + 1, declared_vars_in_scope.copy())
                )
            ...
            output_lines.extend(
                _generate_statements(stmt.body.statements, code, ir_function, indent_level + 1, declared_vars_in_scope.copy())
            )
            output_lines.append(f"{indent}}}")
            ...
            output_lines.extend(
                _generate_statements(stmt.condition.statements, code, ir_function, indent_level + 1, declared_vars_in_scope.copy())
            )
            output_lines.append(f"{indent}}}")
            ...
            if stmt.value:
                if isinstance(stmt.value, IRLocal) and stmt.value.type.resolve(code).kind.value == Type.Kind.VOID.value:
                    output_lines.append(f"{indent}return; // implicit void return from reg{ir_function.locals.index(stmt.value) + 1}")
                else:
...
def pseudo(ir_func: IRFunction) -> str:
    ...
    func_header = f"{_indent_str(base_indent)}{static_kw}function {func_name_str}({params_joined_str}):{return_type_str} {{"
