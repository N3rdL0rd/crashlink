        ...
        if isinstance(stmt, IRBlock):  # Nested block, usually from if/else/loop bodies
            ...
            output_lines.extend(
                _generate_statements(
                    stmt.true_block.statements, code, ir_function, indent_level + 1, declared_vars_in_scope.copy()
                )
            )
            if stmt.false_block and stmt.false_block.statements:
                ...
                output_lines.extend(
                    _generate_statements(
                        stmt.false_block.statements, code, ir_function, indent_level + 1, declared_vars_in_scope.copy()
                    )
                )
            ...
            output_lines.extend(
                _generate_statements(
                    stmt.body.statements, code, ir_function, indent_level + 1, declared_vars_in_scope.copy()
                )
            )
            output_lines.append(f"{indent}}}")
            ...
            output_lines.extend(
                _generate_statements(
                    stmt.condition.statements, code, ir_function, indent_level + 1, declared_vars_in_scope.copy()
                )
            )
            output_lines.append(f"{indent}}}")
