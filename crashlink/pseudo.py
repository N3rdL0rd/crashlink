...
def _expression_to_haxe(
    ...
    if isinstance(expr, IRLocal):
        ...
        inner = _expression_to_haxe(expr.target, code, ir_function)
        # hl.Ref wrappers are compiler-generated by HashLink for value-type
        # arguments that need boxing/addressing. Rendering them as `new
        # hl.Ref(x)` produces Haxe that does not match the original source
        # for helpers like `ArrayDyn.alloc(..., true)`. Keep the underlying
        # constant visible when the wrapped value is a constant or a local
        # that is already a plain compiler-materialized boolean.
        if isinstance(expr.target, (IRConst, IRLocal)):
            return inner
